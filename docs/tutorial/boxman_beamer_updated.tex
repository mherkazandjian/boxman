...
% --- Colors & typography tweaks
\definecolor{BoxmanBlue}{RGB}{25,55,109}
\setbeamercolor{structure}{fg=BoxmanBlue}
\setbeamercolor{frametitle}{fg=BoxmanBlue}
\setbeamerfont{frametitle}{series=\bfseries}
\setbeamertemplate{itemize item}{\small\raise0.15em\hbox{\textbullet}}
...
% --- Boxman CLI (v0.10) highlights
\begin{frame}[fragile]{Boxman v0.10 CLI: what you can do}
\begin{itemize}
  \item \textbf{Create templates from cloud images}: \texttt{create-templates}
  \item \textbf{Bring infra up/down}: \texttt{up}, \texttt{down} (\texttt{--suspend})
  \item \textbf{Inspect \& access}: \texttt{ps}, \texttt{ssh} (\texttt{--cluster})
  \item \textbf{Snapshots}: \texttt{snapshot take/list/restore/delete} and \texttt{restore} (latest)
  \item \textbf{Run tasks}: \texttt{run} (named tasks from \texttt{conf.yml})
  \item \textbf{Move VMs}: \texttt{export} / \texttt{import}
  \item \textbf{Runtime choice}: \texttt{--runtime local|docker}
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Quick command cheat sheet}
\begin{lstlisting}[style=boxman]
# Provision (local runtime by default)
boxman provision

# Idempotent "make it running"
boxman up

# Pause or save state
boxman down          # save state
boxman down --suspend

# Snapshot workflow
boxman snapshot take --name baseline
boxman snapshot list
boxman snapshot restore --name baseline
boxman restore       # restore all VMs to latest snapshot

# Inspect + SSH
boxman ps
boxman ssh           # gateway VM
boxman ssh node02
\end{lstlisting}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    ...
    \item \textbf{Python 3.10+} (recommended: 3.10/3.11/3.12)
  \end{itemize}
