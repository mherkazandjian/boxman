...
| D3 | Snapshot semantics | **Implemented in Phase 7 ([#55](https://github.com/mherkazandjian/boxman/issues/55)).** `docker commit`-backed: `take` commits + tags `boxman/<project>_<box>:<snap>`; `restore` regenerates the compose file with snapshot tags + `up --force-recreate`; `list`/`delete` = image ls/rmi + metadata JSON (`snapshots.json`) in the cluster workdir. **Volumes are not snapshotted** — warned on every `take` and documented in [config-schema.md](./config-schema.md#snapshots-and-container-state-decision-d3). |
