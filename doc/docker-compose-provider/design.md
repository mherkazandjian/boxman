...
| D3 | Snapshot semantics | **Implemented in Phase 7 ([#55](https://github.com/mherkazandjian/boxman/issues/55)).** `docker commit`-backed: `take` commits + tags `boxman/<project>_<cluster>_<box>:<snap>` (the repo carries the compose project, so clusters can't collide) and refuses a duplicate name; `restore` regenerates the compose file with snapshot tags + `up --force-recreate`; `restore` pre-validates that every recorded image still exists; `list`/`delete` = image ls/rmi + metadata JSON (`snapshots.json`) in the cluster workdir. A narrowed `--vms` skips dc clusters (`--cluster` is the container knob). **Volumes are not snapshotted** — warned on every `take` and documented in [config-schema.md](./config-schema.md#snapshots-and-container-state-decision-d3). |
